...
export async function GET(request: Request) {
  ...
  try {
    ...
    console.error("Facebook OAuth callback could not load workspace.", error);
  ...
  if (url.searchParams.get("error") || metaErrorMessage) {
    return clearStateCookie(
      redirectToConnections(
        request,
        "oauth-failed",
        metaErrorMessage ?? "Meta did not approve the Facebook connection.",
      ),
    );
  }
  ...
  if (!code) {
    return clearStateCookie(
      redirectToConnections(
        request,
        "missing-code",
        "Meta did not return a Facebook connection code.",
      ),
    );
  }
  ...
  const stateResult = verifyFacebookOAuthState({
    state: url.searchParams.get("state"),
    nonce: getCookieValue(request.headers.get("cookie"), facebookOAuthStateCookieName),
    userId: session.user.id,
  ...
  try {
    result = await completeFacebookOAuthCallback({
      code,
      workspaceId: workspace.id,
  ...
  if (!result.success) {
    console.error("Facebook OAuth callback returned a handled failure.", {
...
function redirectToConnections(request: Request, facebookStatus: string, message?: string) {
  const url = new URL("/connections", request.url);
  url.searchParams.set("facebook", facebookStatus);

  if (message) {
    url.searchParams.set("facebook_message", message);
  }
...
function clearStateCookie(response: NextResponse) {
  response.cookies.set(facebookOAuthStateCookieName, "", {
    httpOnly: true,
    maxAge: 0,
    path: "/",
    sameSite: "lax",
